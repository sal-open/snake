...
void Snake::checks(pair window) {
    if (player->head.x <= window.x - WIDTH || player->head.x >= (window.x) - 1 ||
        player->head.y <= window.y - HEIGHT || player->head.y >= (window.y) - 1) alive =  false;
}

// Metodo per definire la direzione del serpente

void Snake::get_Input() {
    tcflush(0 ,TCIFLUSH);                   // Clean the standard input buffer
